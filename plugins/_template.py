"""
Drop-in ALFRED plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — ALFRED discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the JarvisUI instance — use player.write_log(f"ALFRED: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"ALFRED: {result_text}")
        except Exception:
            pass
    return result_text
