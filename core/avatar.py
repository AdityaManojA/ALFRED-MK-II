...
* **Lip-sync comes from the audio pipeline, not from the avatar.** `main.py`
  ...
  and ALFRED's own output. The avatar just consumes that number, so there is no
  second audio path to fall out of sync. The mouth only tracks the level while
  ALFRED is *speaking* — during listening the same level drives the aura, so the
...
# to tell speech from room tone. ALFRED's own voice arrives as a per-20 ms
