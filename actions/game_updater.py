...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path  = plist_dir / "com.alfred.gameupdater.plist"
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>com.alfred.gameupdater</string>
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        r1 = subprocess.run(
            ["schtasks", "/Delete", "/TN", "ALFRED_GameUpdater", "/F"],
            capture_output=True, text=True, **_CNW
        )
        r2 = subprocess.run(
    ...
    if is_mac():
        for p in ("com.alfred.gameupdater.plist", "com.jarvis.gameupdater.plist"):
            plist_path = Path.home() / "Library" / "LaunchAgents" / p
    ...
    try:
        existing = subprocess.run(["crontab", "-l"], capture_output=True, text=True)
        lines    = [l for l in existing.stdout.splitlines()
                    if "ALFRED_GameUpdater" not in l and "JARVIS_GameUpdater" not in l]
...
def _get_schedule_status() -> str:
    if is_windows():
        for tn in ("ALFRED_GameUpdater", "JARVIS_GameUpdater"):
    ...
    try:
        ...
        if "ALFRED_GameUpdater" in result.stdout or "JARVIS_GameUpdater" in result.stdout:
            for line in result.stdout.splitlines():
                if "ALFRED_GameUpdater" in line or "JARVIS_GameUpdater" in line:
